            ...
            >
              <Headless.DialogPanel
                className={clsx(
                  className,
                  sizes[size],
                  'row-start-2 w-full min-w-0 rounded-t-3xl bg-white p-[--gutter] shadow-lg ring-1 ring-gray-950/10 @container/dialog [--gutter:theme(spacing.8)] dark:bg-gray-900 dark:ring-white/10 sm:mb-auto sm:rounded-2xl  forced-colors:outline'
                )}
              >
                {children}
              </Headless.DialogPanel>
...
export function DialogTitle({
  ...
  return (
    <Headless.DialogTitle
      ...
      className={clsx(
        className,
        'text-balance text-lg/6 font-semibold text-gray-950 dark:text-white sm:text-base/6'
      )}
    />
  )
}
